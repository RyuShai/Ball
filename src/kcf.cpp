...
double KCF_Tracker::track(cv::Mat &input_gray)
{
    //For the case of multi-tracker, this should be call only once, shared for all tracks
//    cv::Mat input_gray = img.clone();
...
//        img.convertTo(input_gray, CV_32FC1);
    ...
    if (p_pose.cx > input_gray.cols-1) p_pose.cx = input_gray.cols-1;
    if (p_pose.cy < 0) p_pose.cy = 0;
    if (p_pose.cy > input_gray.rows-1) p_pose.cy = input_gray.rows-1;
...
//    std::cout<<"Confident: "<< max_response<<std::endl;
    return max_response;
}
