...
class MultiCameraTracker
{
  ...
  private:
	...
	//vector<vector<pair<Point, Point>>> result_final = vector<vector<pair<cv::Point, cv::Point>>>(6);
  ...
  public:
	...
	void objectHandover()
	{
		...
		if (transitRegion_A.contains(Point(finalPoint3D.x, finalPoint3D.y)) && velX > 0)
			STATE = 1;
		if (transitRegion_A.contains(Point(finalPoint3D.x, finalPoint3D.y)) && velX < 0)
			STATE = 3;

		if (transitRegion_B.contains(Point(finalPoint3D.x, finalPoint3D.y)) && velX > 0)
			STATE = 3;
		if (transitRegion_B.contains(Point(finalPoint3D.x, finalPoint3D.y)) && velX < 0)
			STATE = 5;
		...
		{
			if (abs(currCandidates[i]->cameraID - STATE) <= 1)
				currCandidates[i]->isRealBall = true;
		}
	}
	...
	void compute3D_Coords()
	{
		...
		if (detections == 2)
		{
			...
			if (finalPoint3D.x > 0 && finalPoint3D.y > 0 && finalPoint3D.x < 105 && finalPoint3D.y < 68)
			{
				for (auto cc : currCandidates)
					if (cc->isRealBall)
						cc->coords3D.push_back(finalPoint3D);
	...
	inline void drawPoint(Mat &image, ProjCandidate *cand, int rad = 1, int thickness = 1)
	{
		...
		if (cand->coords.size() < 1 || cand->coords.size() > 2000)
			return;
		//if (cand->cameraID + 1 != 3 && cand->cameraID + 1 != 4) return;

		if (thickness == 1)
			circle(image, *(cand->coords.end() - 1), rad, colors[cand->cameraID], 1, CV_AA);
		if (thickness == -1)
			circle(image, *(cand->coords.end() - 1), rad, teamColors[cand->teamID], thickness, CV_AA);
	...
	inline void drawBall(Mat &image, ProjCandidate *cand, int rad = 1, int thickness = 1)
	{
		...
		if (thickness == -1)
			circle(image, *(cand->coords.end() - 1), rad, colors[cand->cameraID], thickness, CV_AA);
	...
	void identifyTruePositive(ofstream &file, int globalFrameCount)
	{
		...
		{
			if (globalFrameCount >= 358)
				file << 0 << " " << framesProcessed << endl;
			return;
		}
		...
		{
			if (globalFrameCount >= 358)
				file << minVal << " " << framesProcessed << endl;
	...
	//=========================================================================================
	cv::Point Inv_Triangulate(Point3d cam, Point3d ball)
	{
